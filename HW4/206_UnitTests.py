...
# Section Day/Time: Thursday/8:30-9:30 am
...
class CardTests(unittest.TestCase):
	def test_cardrank_queen(self):
		card = Card(rank = 12)
		self.assertEqual(card.rank, 'Queen')

	def test_cardrank_ace(self):
		card = Card(rank = 1)
		self.assertEqual(card.rank, 'Ace')

	def test_cardrank_three(self):
		card = Card(rank = 3)
		self.assertEqual(card.rank, 3)

	def test_cardsuit_clubs(self):
		card = Card(suit = 1)
		self.assertEqual(card.suit, 'Clubs')

	def test_cardsuit_hearts(self):
		card = Card(suit = 2)
	...
	# This test checks to see that the default intialization of a Card object is a 2 of diamonds 
	# (when you make a Card object without putting anything for the parameters)
	def test_default_card(self):
		card = Card()
		self.assertEqual(card.suit, 'Diamonds')
		self.assertEqual(card.rank, 2)

	# This test checks to make sure that the Deck str function returns a string
	def test_deck_shuffle(self):
		deck = Deck()
		self.assertEqual(type(str(deck)), str)
